...
object SerialExample {
    ...
    fun main(args: Array<String>) {
        ...
        serial.addListener(SerialDataEventListener { event -> // NOTE! - It is extremely important to read the data received from the
            // serial port.  If it does not get read from the receive buffer, the
            // buffer will continue to grow and consume memory.

            // print out the data received to the console
            try {
                console.println("[HEX DATA]   " + event.hexByteString)
                console.println("[ASCII DATA] " + event.asciiString)
            } catch (e: IOException) {
                e.printStackTrace()
            }
        })
        try {
            ...
            while (console.isRunning) {
                try {
                    ...
                    serial.write("CURRENT TIME: " + Date().toString())

                    // write a individual bytes to the serial transmit buffer
                    serial.write(13.toByte())
                    serial.write(10.toByte())
            ...
            console.println(" ==>> SERIAL SETUP FAILED : " + ex.message)
            return
        }
    }
}
